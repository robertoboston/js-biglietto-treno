...
if(age_user < 18){
    ...
    console.log(n)
    document.getElementById('cost').innerHTML=`${'il prezzo del biglietto è'} ${n}`
}
else if(age_user > 65){
    let tot_under_65= km_tot_user * 0.21 * 0.6;
    let m= tot_under_65.toFixed(2)+euro
    console.log(m)
    document.getElementById('cost').innerHTML=`${'il prezzo del biglietto è'} ${m}`
}
else{
    let tot_middle= km_tot_user * 0.21;
    let p= tot_middle.toFixed(2)+euro
    console.log(p)
    document.getElementById('cost').innerHTML=`${'il prezzo del biglietto è'} ${p}`
}
